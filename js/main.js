...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const verifyButton = form.querySelector('button[hx-post="verify-azure-credentials"]');
    const generateButtonStaging = document.getElementById('certificate-button-staging');
    ...
    [verifyButton, generateButtonStaging, generateButtonProd].forEach(button => {
    ...
    function verify_credentials() {
        ...
        const generateValid = verifyValid && requiredGenerateFields.every(field => {
            const input = form.querySelector(`[name="${field}"]`);
            return input && input.value.trim() !== '';
        });
...
document.body.addEventListener('htmx:beforeRequest', function (evt) {
    if (evt.detail.requestConfig.path.includes('generate-certificate')) {
        ...
        document.querySelectorAll('#certificate-button-staging, #certificate-button-prod').forEach(btn => {
            btn.disabled = true;
    ...
    importApplyBtn.addEventListener("click", function () {
        try {
            ...
            if ("resource_group" in data) {
                form.querySelector("[name='resource_group']").value = data.resource_group;
            }
            if ("email" in data) {
                form.querySelector("[name='email']").value = data.email;
            }
            if ("domain_common_name" in data) {
                form.querySelector("[name='domain_common_name']").value = data.domain_common_name;
            }

            // Handle SANs (remove existing lines, then re-inject)
            if ("subject_alternative_names" in data && Array.isArray(data.subject_alternative_names)) {
